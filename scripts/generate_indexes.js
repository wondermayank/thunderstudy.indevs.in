...
function renderPage(cfg) {
  const schemaBreadcrumbs = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": cfg.name, "item": "https://thunderstudy.indevs.in/" + cfg.slug + "/" }
    ]
  };
  ...
  const schemaCollection = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": cfg.name + " - Free Study Material, Notes & Mock Tests | ThunderStudy",
    "description": cfg.description,
    "url": "https://thunderstudy.indevs.in/" + cfg.slug + "/",
    "inLanguage": "en-IN",
    "publisher": {
  ...
  const cardsHtml = cfg.items.map(item => `
        <a href="${item.url}" class="course-card">
          <div>
            <div class="card-icon-wrap">${icons[item.icon] || icons.book}</div>
...
<head>
  ...
  <meta name="speakable" content="${cfg.quickAnswer.replace(/"/g, '&quot;')}">
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .brand-study {
      background: linear-gradient(135deg, var(--primary), #7560C7);
...
<body>
  ...
  <main>
    <section class="hero-section">
      <span class="eyebrow">${cfg.badge || 'Official Hub'}</span>
      <h1 class="hero-title">${cfg.headline || cfg.name}</h1>
      <p class="hero-summary">${cfg.subheadline || cfg.description}</p>
...
const dataRaw = fs.readFileSync(path.join(__dirname, 'hub_data.json'), 'utf8').trim().replace(/^\uFEFF/, '');
const configs = JSON.parse(dataRaw);

configs.forEach(page => {
  const targetDir = path.join(rootDir, page.dir);
  ...
  console.log('Successfully generated ' + page.dir + '/index.html');
});
